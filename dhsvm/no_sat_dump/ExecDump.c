  ...
  VEGPIX *Veg, int NSoil, int NCanopyStory, OPTIONSTRUCT *Options, int flag)
{
  ...
  float W_veg;      /* precipitation after interception */
  float P_int;      /* precipitation after interception */
  float tmp;
  float tmp2;
  ...
  P_int = Precip->RainAfterInt + Precip->SnowAfterInt;  /* Precipitation after interception*/
  W_veg = P_int + deltaSWE + Snow->VaporMassFlux;
  ...
  if (first == 1) {
    ...
    fprintf(OutFile->FilePtr, "W_veg(mm) ");
    fprintf(OutFile->FilePtr, "Precip(m) ");
    fprintf(OutFile->FilePtr, "Snow(m) ");
    fprintf(OutFile->FilePtr, "P_int(m) ");
    fprintf(OutFile->FilePtr, "deltaSWE(m) ");
    fprintf(OutFile->FilePtr, "VaporFlux(m) ");
    fprintf(OutFile->FilePtr, "Outflow(m) ");
  ...
  fprintf(OutFile->FilePtr, " %g ", W_veg*1000);
  fprintf(OutFile->FilePtr, " %g ", Precip->Precip);
  fprintf(OutFile->FilePtr, " %g ", Precip->SnowFall);
  fprintf(OutFile->FilePtr, " %g ", P_int);
  fprintf(OutFile->FilePtr, " %g ", Snow->OldSwq - Snow->Swq);
  fprintf(OutFile->FilePtr, " %g ", Snow->VaporMassFlux);
